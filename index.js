          ...
          if (!((s4dmessage.content) == '$bal')) {
            try{
              ...
              var embed = new Discord.MessageEmbed()
                ...
                embed.setDescription(([s4dmessage.mentions.members.first(),' currently has ',s4d.database.get(String(('coins-' + String((s4dmessage.mentions.members.first()).id)))),' coins in their account.'].join('')));
                (s4dmessage.channel).send({embeds:[embed]});
        ...
        if (((s4dmessage.content) || '').startsWith('$editcoins' || '')) {
          ...
          if ((s4dmessage.member).permissions.has(Permissions.FLAGS.MANAGE_GUILD)) {
            try{
              ...
              var embed = new Discord.MessageEmbed()
                 embed.setColor((colourRandom()));
                embed.setDescription(([s4dmessage.author,' reset the coin count for ',s4dmessage.mentions.members.first(),' to 0'].join('')));
                (s4dmessage.channel).send({embeds:[embed]});
             ...
             s4d.message = collected.first();
              ...
              if (give_answer % 2 === 0 || give_answer % 2 === 1) {
                if (coins >= give_answer) {
                  ...
                  var embed = new Discord.MessageEmbed()
                     embed.setColor((colourRandom()));
                    embed.setDescription(([s4dmessage.author,' you successfully gave ',give_answer,' coins to ',s4dmessage.mentions.members.first()].join('')));
                    (s4dmessage.channel).send({embeds:[embed]});
